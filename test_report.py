...
def safe_print(text: str) -> None:
    """
    Print sécurisé qui gère les consoles ne supportant pas tous les emojis
    (par ex. Windows cp1252). Si un UnicodeEncodeError survient, on remplace
    les symboles problématiques par des équivalents ASCII.
    """
    try:
        print(text)
...
try:
    ...
    safe_print("❌ Erreur: Le module PyYAML n'est pas installé.")
    safe_print("   Installez-le avec: pip install pyyaml")
    sys.exit(1)
...
def load_test_list(yaml_path='test_list.yaml'):
    ...
    try:
        ...
        safe_print(f"❌ Erreur: Fichier {yaml_path} introuvable.")
        sys.exit(1)
    except yaml.YAMLError as e:
        safe_print(f"❌ Erreur lors de la lecture du YAML: {e}")
        sys.exit(1)
...
def load_test_results(json_path='result_test_auto.json'):
    ...
    try:
        ...
        safe_print(f"⚠️  Avertissement: Erreur lors de la lecture du JSON: {e}")
        return None
...
def print_test_report(tests, results_data):
    ...
    if results_data:
        ...
        safe_print("⚠️  Fichier non trouvé ou invalide - tous les tests auto seront marqués 'Not found'\n")
    ...
    for test in tests:
        ...
        safe_print(f"{test_number} | {test_type:6} | {emoji} {status_text}")
...
def print_statistics(stats, total_tests):
    ...
    safe_print(f"✅ Passed + 🫱 Manual: {success_count} ({success_pct:.1f}%)")


def main():
    """Point d'entrée principal du script."""
    safe_print("=" * 70)
    safe_print("TEST REPORT - Rapport des tests")
    safe_print("=" * 70)

    # Charger les tests depuis le YAML
    tests = load_test_list()
    if not tests:
        safe_print("❌ Aucun test trouvé dans le fichier YAML.")
        sys.exit(1)
